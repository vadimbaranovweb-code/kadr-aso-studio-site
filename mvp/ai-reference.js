...
export function buildReferenceRequest({slides,references,locale,subtitleEnabled=true}){
 ...
 return {model:REFERENCE_MODEL,store:false,max_output_tokens:14000,instructions,input:[{role:'user',content}],text:{format:{type:'json_schema',name:'aso_reference_composition',strict:true,schema}}};
}
...
export function validateReferenceResult(value,slides){
 ...
 return {summary:value.summary,background:{mode:bg.mode,colors:[...bg.colors],angle:bg.angle,opacities:[1,1]},textColor:value.textColor,slides:rows};
}
export function applyReference(project,plan){
 ...
 for(const [i,slide] of next.slides.entries()){
  Object.assign(slide,plan.slides[i],{titleExample:false,subtitleExample:false,titleHighlight:'',subtitleHighlight:''});
