...
export function validateProject(value){
  ...
  if(value?.app!=='kadr-mvp'||![1,2,3,4,5,6].includes(value.version))throw new Error('Этот формат проекта не поддерживается. Выберите файл Кадра MVP.');
  ...
  const slides=value.slides.map(s=>{
    ...
    return {id:s.id,image:s.image,name:String(s.name||'Экран').slice(0,150),width:s.width,height:s.height,title:s.title,subtitle:s.subtitle,titleSize:number(s.titleSize,34,18,56),subtitleSize:number(s.subtitleSize,17,12,28),titleExample:s.titleExample===true,subtitleExample:s.subtitleExample===true,screenFit:screenFit(s.screenFit),screenScale:number(s.screenScale,1,1,3),screenX:number(s.screenX,0,-1,1),screenY:number(s.screenY,0,-1,1),composition:validateComposition(s.composition),...validateCopyDecoration(s),copies:validateCopies(s.copies),screens:validateScreens(s.screens,imageBytes)};
  });
  return {app:'kadr-mvp',version:6,appleFormat:value.appleFormat??'440',exportScale:number(value.exportScale,3,1,3),iphoneDevice,phoneAlignment:value.phoneAlignment??'text',phoneScale:value.phoneScale==null?null:number(value.phoneScale,1,.6,1.8),phoneY:number(value.phoneY,0,-.2,.3),textColor:color(value.textColor,null),textOpacity:number(value.textOpacity,1,0,1),locale:value.locale??'source',name:String(value.name||'Мой комплект').slice(0,80),store,androidDevice,frame:value.frame,deviceColor:value.deviceColor??null,layout:value.layout,cropScale:number(value.cropScale,1,.6,1.05),cropRaise:number(value.cropRaise,0,0,.2),subtitleEnabled:value.subtitleEnabled!==false,background:{mode:bg.mode,angle:bg.angle===null||bg.angle===undefined?null:number(bg.angle,20,0,360),colors:[...bg.colors],opacities:validateOpacities(bg.opacities)},slides};
}
...
export function cloneProject(p){return {...p,background:{...p.background,colors:[...p.background.colors],opacities:[...(p.background.opacities||[1,1])]},slides:p.slides.map(s=>({...s,...copyOf(s),composition:cloneComposition(s.composition),screens:Object.fromEntries(Object.entries(s.screens||{}).map(([locale,screen])=>[locale,{...screen}])),copies:Object.fromEntries(Object.entries(s.copies||{}).map(([locale,copy])=>[locale,copyOf(copy)]))}))};}
