...
export function validateProject(value){
  ...
  if(value?.app!=='kadr-mvp'||![1,2,3].includes(value.version))throw new Error('Этот формат проекта не поддерживается. Выберите файл Кадра MVP.');
...
function validateCopyDecoration(s){const out={};for(const field of ['title','subtitle']){const h=s[field+'Highlight']??'';if(typeof h!=='string'||h.length>160)throw new Error('Некорректное выделение текста.');out[field+'Highlight']=h;out[field+'HighlightColor']=color(s[field+'HighlightColor'],'#2563EB');}return out;}
const COPY_KEYS=['title','subtitle','titleExample','subtitleExample','titleHighlight','subtitleHighlight','titleHighlightColor','subtitleHighlightColor'];
export function copyOf(s){return Object.fromEntries(COPY_KEYS.map(k=>[k,s[k]]));}
...
export function switchLocale(project,locale){if(!Object.hasOwn(LOCALES,locale))throw new Error('Неизвестный язык.');if(locale===project.locale)return;for(const s of project.slides){s.copies[project.locale]=copyOf(s);Object.assign(s,s.copies[locale]||s.copies.source||copyOf(s));}project.locale=locale;}
export function applyCopy(project,locale,rows){if(rows.length!==project.slides.length||rows.some((r,i)=>r.id!==project.slides[i].id))throw new Error('Состав комплекта изменился. Сгенерируйте тексты заново.');switchLocale(project,locale);for(const [i,s] of project.slides.entries()){Object.assign(s,{title:rows[i].title,subtitle:rows[i].subtitle,titleExample:false,subtitleExample:false,titleHighlight:'',subtitleHighlight:''});s.copies[locale]=copyOf(s);}}

// Copy mutable settings/copybooks while sharing immutable image strings across undo entries.
export function cloneProject(p){return {...p,background:{...p.background,colors:[...p.background.colors],opacities:[...(p.background.opacities||[1,1])]},slides:p.slides.map(s=>({...s,copies:Object.fromEntries(Object.entries(s.copies||{}).map(([locale,copy])=>[locale,{...copy}]))}))};}
