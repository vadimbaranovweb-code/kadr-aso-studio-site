...
function connectionControl(){return '<div data-ai-connection-host></div>';}
function aiStepControls(){return section('Подключение',connectionControl())+section('Сборка по референсу',`<p class="control-hint">Добавьте примеры из App Store. AI предложит композицию с вашими экранами.</p><button class="button primary" data-ai-reference ${ai.hasKey()?'':'disabled'}>Собрать по референсу</button>`)+section('Тексты по содержимому',`<p class="control-hint">Заголовки и подзаголовки на основе интерфейса приложения.</p><button class="button" data-ai-generate ${ai.hasKey()?'':'disabled'}>Сгенерировать тексты</button>`)+`<p class="control-hint ai-step-note">Запросы отправляются только по вашей команде и оплачиваются через API-аккаунт. Результат можно проверить перед применением.</p>`;}
function typographyControl(){const slide=selectedSlide(),index=project.slides.indexOf(slide);return `<p class="control-hint">Текст редактируется прямо на кадрах.${slide?' Размеры: слайд '+(index+1)+'.':''}</p>`+fontControl('title')+(project.subtitleEnabled?fontControl('subtitle'):'')+`<button class="button small" data-apply-sizes>Применить размеры ко всем</button>`;}

function backgroundControl(){return `<span class="field-label">Фон</span><div class="segmented"><button data-bg-mode="solid" aria-pressed="${project.background.mode==='solid'}">Заливка</button><button data-bg-mode="gradient" aria-pressed="${project.background.mode==='gradient'}">Градиент</button></div><div class="color-stops">${colorChip(0)}${project.background.mode==='gradient'?colorChip(1):''}</div>${project.background.mode==='gradient'?`<label class="angle-control">Угол градиента <output data-angle-output>${project.background.angle??20}°</output><input aria-label="Угол градиента" data-angle type="range" min="0" max="360" value="${project.background.angle??20}"></label>`:''}<span class="field-label">Цвет текста</span><div class="color-stops"><button class="color-chip" data-open-color="text" aria-label="Цвет текста"><i style="background:${displayedTextColor()}"></i><span>${displayedTextColor().toUpperCase()}</span></button><button class="button quiet small" data-auto-text aria-label="Автоматический цвет текста">Авто</button></div>`;}
...
function render(){
 ...
 lastRenderedStep=step;ai.mountConnection(root.querySelector('[data-ai-connection-host]'));
...
function paint(){
 for(const canvas of root.querySelectorAll('[data-slide-canvas]')){const i=project.slides.findIndex(v=>v.id===canvas.dataset.slideCanvas),slide=project.slides[i];if(!slide)continue;const editing=activeText?.id===slide.id?activeText.field:null,l=renderSlide(canvas,project,slide,images.get(screenFor(slide,project.locale).image),i,{scale:.55});const shell=canvas.parentElement,factor=shell.clientWidth/l.w;const hit=shell.querySelector('[data-phone-hit]');if(hit)Object.assign(hit.style,{left:l.phone.x/l.w*100+'%',top:l.phone.y/l.h*100+'%',width:l.phone.width/l.w*100+'%',height:l.phone.height/l.h*100+'%',transform:'rotate('+(l.phone.rotation||0)+'deg)'});
...
async function exportPack(){
  ...
  try{
    ...
    const jobs=packEntries(snapshot,[...exportLocales]);for(const {variant,slide,index,name} of jobs){
      const canvas=document.createElement('canvas');renderSlide(canvas,variant,slide,images.get(screenFor(slide,variant.locale).image),index,{scale:snapshot.store==='apple'?snapshot.exportScale/3:1});
...
root.addEventListener('click',e=>{const el=e.target.closest('button');if(!el||busy)return;if(el.hasAttribute('data-phone-hit')){if(step!==4)return;if(Date.now()<suppressClickUntil)return;activeText=null;activePhone=el.dataset.phoneHit;selectedSlideId=activePhone;render();revealControl('.phone-toolbar');}
...
root.addEventListener('input',e=>{if(busy)return;const el=e.target;if(el.hasAttribute('data-screen')&&activePhone){changed(()=>screenSettings(project.slides.find(s=>s.id===activePhone),project.locale)[el.dataset.screen]=+el.value/100,{remember:false,rebuild:false});root.querySelector(`[data-screen-output="${el.dataset.screen}"]`).textContent=el.value+'%';}});
...
root.addEventListener('focusin',event=>{if(event.target.matches('[data-composition]'))checkpoint();if(event.target.matches('[data-text]'))textEditCheckpoint=true;if(event.target.matches('[data-name],[data-phone],[data-screen],[data-angle],[data-size-field]'))checkpoint();if(activeText&&event.target.matches('[data-panel-text]')){activeText=null;render();}});
function sizeTargets(el){return el.hasAttribute('data-all')?project.slides:[selectedSlide()].filter(Boolean);}
root.addEventListener('input',e=>{const el=e.target;if(busy)return;if(el.hasAttribute('data-size-field')&&el.value!==''&&el.validity.valid)changed(()=>sizeTargets(el).forEach(s=>s[el.dataset.sizeField+'Size']=+el.value),{remember:false,rebuild:false});});
root.addEventListener('click',e=>{const el=e.target.closest('button');if(!el||busy)return;if(el.hasAttribute('data-size-step')){const field=el.dataset.sizeField;changed(()=>sizeTargets(el).forEach(s=>s[field+'Size']=Math.max(field==='title'?18:12,Math.min(field==='title'?56:28,s[field+'Size']+Number(el.dataset.sizeStep)))));}if(el.hasAttribute('data-apply-sizes')){const current=selectedSlide();if(current)changed(()=>project.slides.forEach(s=>{s.titleSize=current.titleSize;s.subtitleSize=current.subtitleSize;}));notice('Размеры текста применены ко всем слайдам.');}});
...
projectInput.addEventListener('change',async()=>{
  ...
  try{
    ...
    changed(()=>{project=next;step=4;activeText=null;activePhone=null;selectedSlideId=null;canvasScroll=[0,0];panelScroll={};root.replaceChildren();exportLocales=new Set([project.locale]);exportBaseLocale=project.locale;});notice('Проект открыт.');
