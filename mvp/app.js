import {createAIOnboarding} from './ai-onboarding.js';
...
import {availableLocales,hasLocale,packVariants,packEntries,withCopies} from './pack-plan.js';
import {marksFor,setMark,editMarks} from './text-marks.js';
import {STORES,APPLE_FORMATS,formatFor,exportFormats,LOCALES,switchLocale,cloneProject,createProject,createSlide,validateProject,moveSlide,exportIssues,hasExamples,slug} from './model.js';
import {ANDROID_DEVICES,IPHONE_DEVICES,deviceFor} from './devices.js';
import {createAIDialog} from './ai-dialog.js';
...
const autosave=createAutosave({store,validate:validateProject,getSnapshot:()=>({project,view:{step,wizardVersion:WIZARD_VERSION,exportLocales:[...exportLocales],exportBaseLocale}}),restore:async snapshot=>{
  ...
  exportLocales=new Set(Array.isArray(view.exportLocales)?view.exportLocales.filter(l=>Object.hasOwn(LOCALES,l)):[project.locale]);exportBaseLocale=hasLocale(project,view.exportBaseLocale)?view.exportBaseLocale:project.locale;
...
function backgroundControl(){
 const tabs=`<div class="segmented"><button data-background-tab="color" aria-pressed="${backgroundTab==='color'}">Цвет</button><button data-background-tab="generate" aria-pressed="${backgroundTab==='generate'}">Сгенерировать</button></div>`;
 const generation=`<p class="control-hint">Создайте фон по описанию и проверьте его на всём комплекте.</p><button class="button" data-ai-background ${ai.hasKey()?'':'disabled'}>Фон по описанию</button>${!ai.hasKey()?'<button class="button quiet" data-step="3">Подключить AI</button>':''}`;
 ...
 const asset=project.background.asset?`<div class="background-thumb"><img src="${project.background.asset.image}" alt="Сгенерированный фон"><button class="button small" data-bg-mode="${project.background.mode==='image'?'solid':'image'}">${project.background.mode==='image'?'Выключить фон':'Использовать фон'}</button></div>`:'';
 ...
 return tabs+(backgroundTab==='generate'?generation:fill)+asset+text;
}
...
function poseControl(){
 ...
 return `<div class="pose-controls"><strong>Мокап · слайд ${project.slides.indexOf(s)+1}</strong><div class="segmented"><button data-pose-mode="flat" aria-pressed="${!three}">2D</button><button data-pose-mode="3d" aria-pressed="${three}" ${project.store!=='apple'||project.frame!=='device'?'disabled':''}>3D · iPhone 17</button></div>${range('rotation','Поворот',-180,180)}${three?range('yaw','Разворот в 3D',-180,180)+range('pitch','Наклон в 3D',-80,80)+range('perspective','Перспектива',20,60)+range('reflection','Блик',0,1,.01):''}<button class="button small" data-reset-pose>Сбросить поворот</button>${three?'<p class="control-hint">Объёмная модель iPhone 17. Цвет корпуса и исходный UI сохраняются.</p>':''}</div>`;
}
...
const ai=createAIDialog({getProject:()=>project,getRevision:()=>revision,getImage:key=>images.get(key),onApply:(entries,options)=>{const next=validateProject(withCopies(project,entries,options));changed(()=>{project=next;step=4;activeText=null;activePhone=null;});notice(entries.length>1?'Переводы готовы. Проверьте языки перед выгрузкой.':'Тексты применены. Их можно поправить прямо на кадрах.');},onLocalize:async(next,expected,locales)=>{await cacheImages(next);if(revision!==expected)throw new Error('Комплект изменился. Подготовьте перевод заново.');changed(()=>{project=next;step=5;activeText=null;activePhone=null;for(const l of locales)exportLocales.add(l);});notice('Переводы готовы. Проверьте языки на холсте перед экспортом.');},onReference:async(next,expected)=>{await cacheImages(next);if(revision!==expected)throw new Error('Комплект изменился. Соберите предложение заново.');changed(()=>{project=validateProject(next);step=4;activeText=null;activePhone=null;selectedSlideId=project.slides[0]?.id;});notice('Композиция применена. Тексты редактируются на кадрах, расположение — в панели.');},onBackground:async(next,expected)=>{if(expected!==revision)throw new Error('Комплект изменился.');await cacheImages(next);if(expected!==revision)throw new Error('Комплект изменился.');changed(()=>project=next);notice('Фон применён ко всему комплекту.');},onKeyChange:()=>{if(step===3)aiFlow.refresh();else render();}});
const aiFlow=createAIOnboarding({ai,getProject:()=>project,getRevision:()=>revision,
 onBusy:value=>{busy=value;root.inert=value;root.setAttribute('aria-busy',String(value));document.querySelector('.header-actions').inert=value;for(const b of document.querySelectorAll('.header-actions button'))b.disabled=value;},
 onContinue:()=>goStep(4),
 onChange:({disabled,summary,cost})=>{if(step===3){const summaryNode=root.querySelector('[data-ai-summary]'),costNode=root.querySelector('[data-ai-cost]');if(summaryNode)summaryNode.textContent=summary;if(costNode)costNode.textContent=cost;const next=root.querySelector('[data-next]');if(next)next.disabled=disabled;}},
 onApply:async(next,expected,signal)=>{await cacheImages(next);if(signal.aborted)throw new Error('Сборка отменена.');if(expected!==revision)throw new Error('Комплект изменился. Запустите сборку заново.');changed(()=>{project=validateProject(next);step=4;activeText=null;activePhone=null;selectedSlideId=project.slides[0]?.id;});notice('Комплект готов. Тексты можно править на кадрах. «Отменить» вернёт предыдущее оформление.');}
});
root.addEventListener('click',e=>{if(busy)return;if(e.target.closest('[data-ai-key]')){colorPicker?.close();ai.connect();}if(e.target.closest('[data-ai-generate]')){colorPicker?.close();ai.open();}if(e.target.closest('[data-ai-reference]')){colorPicker?.close();ai.open({mode:'reference'});}if(e.target.closest('[data-localize]'))ai.open({mode:'localize',locales:step===5?[...exportLocales]:[],sourceLocale:exportBaseLocale});});
root.addEventListener('change',e=>{if(busy)return;if(e.target.hasAttribute('data-export-scale'))changed(()=>project.exportScale=+e.target.value);if(e.target.hasAttribute('data-export-locale')){const l=e.target.dataset.exportLocale;e.target.checked?exportLocales.add(l):exportLocales.delete(l);readyDownload=null;autosave.changed();render();}});
function goStep(next){if(!canVisit(next,project.slides.length>0))return;activeText=null;activePhone=null;colorPicker?.close();if(next>=5&&step<5){exportBaseLocale=project.locale;if(!exportLocales.size)exportLocales.add(project.locale);}step=next;autosave.changed();render();}
function openExport(){goStep(5);}
function localizationControls(){const available=availableLocales(project);return section('Языки',`<p class="control-hint">Основа: ${LOCALES[exportBaseLocale]}. Выберите нужные версии или продолжите без перевода.</p><div class="export-languages">${Object.entries(LOCALES).map(([l,name])=>`<label><input type="checkbox" data-export-locale="${l}" ${exportLocales.has(l)?'checked':''}><span>${name}</span><small>${available.includes(l)?'Готово':'Перевести'}</small></label>`).join('')}</div>`)+section('Перевод',ai.hasKey()?`<button class="button primary" data-localize>Локализовать выбранное</button><p class="control-hint">Тексты и, при необходимости, интерфейс. Проверка результата перед применением.</p>`:`<p class="control-hint">Для автоматического перевода подключите ключ на шаге AI.</p><button class="button" data-step="3">Перейти к подключению</button>`)+`<button class="button" data-return-editor>Редактировать текущий язык</button><p class="control-hint">После изменения основы переводы нужно обновить.</p>`;}
function exportControls(){const available=availableLocales(project),missing=[...exportLocales].filter(l=>!available.includes(l));return section('В комплекте',`<p class="export-summary">${screenCount(project.slides.length)}</p><p class="control-hint">${[...exportLocales].map(l=>esc(LOCALES[l])).join(' · ')||'Языки не выбраны'}</p><button class="button quiet small" data-step="5">Изменить языки</button>${missing.length?`<p class="issue">Не готовы: ${missing.map(l=>esc(LOCALES[l])).join(', ')}. Вернитесь к локализации.</p>`:''}`)+section('Размеры',project.store==='apple'?`<select data-export-scale aria-label="Масштаб экспорта"><option value="3" ${project.exportScale===3?'selected':''}>3× · для App Store</option><option value="1" ${project.exportScale===1?'selected':''}>1× · рабочие размеры</option></select><p class="control-hint">${project.exportScale===3?'1320 × 2868 и 1242 × 2688':'440 × 956 и 414 × 896'} · оба размера в ZIP.</p>`:'<p class="control-hint">Android · 1080 × 1920</p>')+`<div id="issues" class="issue" hidden></div><button class="button primary" data-export>Скачать ZIP</button>${readyDownload?`<div class="download-ready"><p>${readyDownload.count} PNG готовы.</p><button class="button" data-download>Скачать ещё раз</button></div>`:''}`;}
root.addEventListener('click',e=>{const b=e.target.closest('button');if(!b||busy)return;if(b.hasAttribute('data-open-export'))openExport();if(b.hasAttribute('data-return-editor'))goStep(4);if(b.hasAttribute('data-step'))goStep(Number(b.dataset.step));if(b.hasAttribute('data-review-locale'))changed(()=>switchLocale(project,b.dataset.reviewLocale));});
...
function inspector(){
 ...
 if(step===3)return section('Ваш комплект',`<p class="control-hint">${screenCount(project.slides.length)} · ${project.store==='apple'?'App Store':'Android'}</p><p class="control-hint">Все тексты и оформление можно изменить в редакторе.</p>`)+section('Результат',`<p class="control-hint" data-ai-summary></p><p class="control-hint" data-ai-cost></p>`);
 if(step===5)return localizationControls();
 if(step===6)return exportControls();
 return section('Расположение',orderControl())+section('Тексты',typographyControl()+subtitleControl())+section('Телефон',framesControl()+deviceControl()+cropControl())+section('Фон и цвет текста',backgroundControl())+(activePhone?section('Выбранный телефон',poseControl()+phoneToolbar()):'')+compositionControls(project,selectedSlideId||selectedSlide()?.id)+section('Платформа',`<select aria-label="Платформа" data-store-select>${platformOptions()}</select>`+formatControl());
}
function workspace(){const spec=formatFor(project),available=availableLocales(project),editable=step===4;return `<div class="workspace" data-workflow-step="${step}">
 <nav class="workflow-nav" aria-label="Этапы создания"><span class="nav-caption">Создание комплекта</span><ol>${labels.map((name,i)=>`<li><button data-step="${i+1}" ${canVisit(i+1,project.slides.length>0)?'':'disabled'} ${i+1===step?'aria-current="step"':''}><span class="nav-number">${String(i+1).padStart(2,'0')}</span>${name}</button></li>`).join('')}</ol><p class="nav-footnote">${step===3?'AI — необязательный шаг. Можно оформить комплект вручную.':'Проект сохраняется в этом браузере.'}</p></nav>
 <section class="workspace-center"><header class="workspace-toolbar"><div class="workspace-title"><input class="project-name" data-name aria-label="Название комплекта" maxlength="80" value="${esc(project.name)}"><span class="workspace-meta">${project.store==='apple'?'App Store':'Android'} · ${spec.workingWidth||spec.width} × ${spec.workingHeight||spec.height}</span></div><div class="canvas-tools">${step>=3?`<button class="button quiet icon-action" data-upload aria-label="Добавить экраны" title="Добавить экраны">${icon('plus')}</button><button class="button quiet icon-action" data-undo ${history.length?'':'disabled'} aria-label="Отменить" title="Отменить">${icon('undo')}</button>`:''}${available.length>1&&step>=4?`<select data-editor-locale aria-label="Язык комплекта">${available.map(l=>`<option value="${l}" ${project.locale===l?'selected':''}>${LOCALES[l]}</option>`).join('')}</select>`:''}</div></header>
 ${step<=3?`<div class="onboarding-surface">${setupContent()}</div>`:`<div class="workspace-stage ${editable?'':'review-stage'}"><div class="filmstrip pan-area" tabindex="0" aria-label="Холст комплекта, прокрутка по горизонтали">${project.slides.map(slideCard).join('')}<button class="add-slide ${editable?'':'review-placeholder'}" data-upload ${!editable||project.slides.length>=spec.max?'disabled':''} ${editable?'':'tabindex="-1" aria-hidden="true"'}>${icon('plus')}Добавить экран</button></div><footer class="canvas-status"><span>${screenCount(project.slides.length)}</span><span>${editable?'Текст — клик по заголовку · Холст — потяните в сторону':step===3?'Настройте AI или продолжите вручную':step===5?'Проверьте финальные языковые версии':'PNG · все выбранные языки и размеры'}</span></footer></div>`}</section>
 <aside class="inspector"><header class="inspector-heading"><h1>${labels[step-1]}</h1><span>${step} / ${labels.length}</span></header><div class="inspector-scroll">${inspector()}</div><footer class="step-actions"><button class="button quiet" data-back ${step===1?'disabled':''}>Назад</button>${step<6?`<button class="button primary" data-next ${step===2&&!project.slides.length?'disabled':''}>${step===4?'Локализация':step===5?'К экспорту':'Далее'}${icon('arrow')}</button>`:''}</footer></aside>
 </div>`;}
...
function render(){
 ...
 document.body.classList.add('workspace-active');document.body.classList.toggle('editing',step===4);
 ...
 lastRenderedStep=step;aiFlow.mount(root.querySelector('[data-ai-onboarding-host]'));
...
function updateIssues(){const el=document.getElementById('issues');if(!el)return;const issues=exportIssues(project),ctx=document.createElement('canvas').getContext('2d');let variants;try{variants=step===6?packVariants(project,[...exportLocales]):exportFormats(project);}catch(e){issues.push(e.message);variants=[];}variants.forEach(p=>p.slides.forEach((s,i)=>{if(frameLayout(ctx,p,s).overflow)issues.push(`${LOCALES[p.locale]} · слайд ${i+1}: сократите текст, чтобы он читался и помещался (${formatFor(p).workingWidth||1080}).`);}));el.hidden=!issues.length;el.textContent=issues.join(' ');const button=root.querySelector('[data-export]');if(button)button.disabled=busy||issues.length>0;}
...
async function exportPack(){
  ...
  let variants;try{variants=packVariants(project,[...exportLocales]);}catch(e){notice(e.message,true);return;}
  if(variants.some(hasExamples)&&!await dialog('В комплекте остались тексты-примеры','Их можно заменить прямо на кадрах. Скачать изображения с этими текстами?',{confirm:'Скачать как есть',cancel:'Изменить тексты'}))return;
  activeText=null;
  ...
  try{
    ...
    const jobs=packEntries(snapshot,[...exportLocales]);for(const {variant,slide,index,name} of jobs){
    ...
    readyDownload={count:files.length,description:snapshot.store==='apple'?(snapshot.exportScale===3?'1320 × 2868 и 1242 × 2688':'440 × 956 и 414 × 896'):'1080 × 1920',blob:makeZip(files),name:`${slug(snapshot.name)}-${snapshot.store}-${[...exportLocales].join('-')}.zip`};saveBlob(readyDownload.blob,readyDownload.name);notice('Комплект готов. PNG собраны в один ZIP.');
  }catch(error){notice(error.message||'Не удалось подготовить комплект. Попробуйте снова.',true);}finally{busy=false;render();}
}
root.addEventListener('click',e=>{const el=e.target.closest('button');if(!el||busy)return;if(el.hasAttribute('data-phone-hit')){if(step!==4)return;if(Date.now()<suppressClickUntil)return;activeText=null;activePhone=el.dataset.phoneHit;selectedSlideId=activePhone;render();revealControl('.pose-controls');}
...
root.addEventListener('click',async event=>{
  ...
  if(el.hasAttribute('data-inline')&&step===4)return startInline(el.dataset.id,el.dataset.inline);
  ...
  if(el.hasAttribute('data-background-tab')){backgroundTab=el.dataset.backgroundTab;render();return;}
  if(el.hasAttribute('data-ai-background')){colorPicker?.close();ai.open({mode:'background'});return;}
  ...
  if(el.hasAttribute('data-next')){if(step===3)await aiFlow.run();else goStep(step+1);return;}
  ...
  if(el.hasAttribute('data-original-screen')&&project.locale!=='source')return changed(()=>{const s=project.slides.find(s=>s.id===el.dataset.originalScreen);if(s?.screens)delete s.screens[project.locale];});
...
root.addEventListener('change',event=>{if(busy)return;const el=event.target;if(el.hasAttribute('data-align-phones'))changed(()=>project.phoneAlignment=el.checked?'shared':'text');if(el.hasAttribute('data-editor-locale')){activeText=null;activePhone=null;changed(()=>switchLocale(project,el.value));}if(el.hasAttribute('data-subtitle-toggle'))changed(()=>project.subtitleEnabled=el.checked);if(el.hasAttribute('data-gradient'))changed(()=>project.background.mode=el.checked?'gradient':'solid');if(el.hasAttribute('data-store-select'))changed(()=>project.store=el.value);if(el.hasAttribute('data-device-select')&&Object.hasOwn(project.store==='apple'?IPHONE_DEVICES:ANDROID_DEVICES,el.value))changed(()=>project[project.store==='apple'?'iphoneDevice':'androidDevice']=el.value);});
...
projectInput.addEventListener('change',async()=>{
  ...
  try{
    ...
    aiFlow.reset();changed(()=>{project=next;step=4;activeText=null;activePhone=null;selectedSlideId=null;canvasScroll=[0,0];panelScroll={};root.replaceChildren();exportLocales=new Set([project.locale]);exportBaseLocale=project.locale;});notice('Проект открыт.');
...
document.getElementById('new-project').addEventListener('click',async()=>{if(busy)return;if(!await dialog('Начать новый комплект?','Текущий комплект останется в скачанном файле, если вы его сохранили. Новый комплект заменит локальное автосохранение.',{confirm:'Начать заново'}))return;aiFlow.reset();changed(()=>{project=createProject();step=1;activeText=null;activePhone=null;selectedSlideId=null;canvasScroll=[0,0];panelScroll={};root.replaceChildren();exportLocales=new Set(['source']);exportBaseLocale='source';});notice('Можно создавать новый комплект.');});
