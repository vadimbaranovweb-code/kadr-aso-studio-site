...
function setupContent(){
  ...
  if(step===3)content=`<p class="eyebrow">03 / AI и референс</p><h1>Как соберём комплект?</h1><p class="lead">Подключите AI для работы по референсу или продолжите вручную.</p><div class="ai-onboarding">${aiStepControls()}</div>`;
...
function inspector(){
 ...
 if(step===3)return section('Ваш комплект',`<p class="control-hint">${screenCount(project.slides.length)} · ${project.store==='apple'?'App Store':'Android'}</p><p class="control-hint">AI можно пропустить. Все тексты и оформление доступны для ручных правок.</p>`);
