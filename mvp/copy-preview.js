...
export function mountCopyPreview(container,project,locale,rows,getImage){
 ...
 const shells=rows.map((row,index)=>{
  ...
  const paint=()=>{
   const active=editing?.shell===shell?editing.field:null,s=draft.slides[index],l=renderSlide(canvas,draft,s,getImage(s.image),index,{scale:.3,hideText:active});
   for(const el of shell.querySelectorAll('[data-preview-field]')){const field=el.dataset.previewField,heading=field==='title',t=heading?l.title:l.sub,factor=shell.clientWidth/l.w;Object.assign(el.style,{left:(heading?32:36)/l.w*100+'%',top:(heading?l.titleY:l.subtitleY)/l.h*100+'%',width:(l.w-(heading?64:72))/l.w*100+'%',height:Math.max(t.size*(heading?1.15:1.35),t.lines.length*t.size*(heading?1.15:1.35))*factor+4+'px',fontSize:t.size*factor+'px',fontWeight:heading?'700':'400',color:draft.textColor||textColor(draft.background.colors.slice(0,draft.background.mode==='solid'?1:2).map((c,i)=>compositeColor(c,draft.background.opacities[i]))),opacity:String((heading?1:.8)*(draft.textOpacity??1)),lineHeight:heading?'1.15':'1.35'});}
  };
  for(const field of ['title',...(draft.subtitleEnabled?['subtitle']:[])]){
   ...
   input.addEventListener('input',()=>{rows[index][field]=input.value;draft.slides[index][field]=input.value;paint();},{signal:events.signal});
  }
