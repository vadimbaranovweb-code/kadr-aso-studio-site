...
export function mountCopyPreview(container,project,locale,rows,getImage){
 ...
 const shells=rows.map((row,index)=>{
  ...
  const paint=()=>{
   const active=editing?.shell===shell?editing.field:null,s=draft.slides[index],l=renderSlide(canvas,draft,s,getImage(screenFor(s,draft.locale).image),index,{scale:.3,hideText:active});
